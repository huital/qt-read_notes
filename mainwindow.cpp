...
void MainWindow::slotTreeMenu(const QPoint &pos)
{
    ...
    if (curIndex.isValid()) {
        ...
        if (curIndex.parent() == ui->tView_note->rootIndex()) {
            QAction* actionParent = menu.addAction(QStringLiteral("添加子章节"), this, &MainWindow::slotAdditem);
        }
        if (curIndex.parent().isValid() && curIndex.parent().parent() == ui->tView_note->rootIndex())
        {
            QAction* actionProblems = menu.addAction(QStringLiteral("添加新问题"), this, &MainWindow::slotAddNote);
        }
    }
